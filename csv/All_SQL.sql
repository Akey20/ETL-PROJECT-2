-- This script was generated by a beta version of the ERD tool in pgAdmin 4.
-- Please log an issue at https://redmine.postgresql.org/projects/pgadmin4/issues/new if you find any bugs, including reproduction steps.
BEGIN;


CREATE TABLE IF NOT EXISTS public."BLS"
(
    "Series ID" text COLLATE pg_catalog."default",
    "Year" bigint,
    "Period" text COLLATE pg_catalog."default",
    "Label" text COLLATE pg_catalog."default",
    "Value" double precision,
    "Sector" text COLLATE pg_catalog."default"
);

CREATE TABLE IF NOT EXISTS public."Companies"
(
    year text COLLATE pg_catalog."default",
    rank double precision,
    city text COLLATE pg_catalog."default",
    growth double precision,
    workers double precision,
    company text COLLATE pg_catalog."default",
    state_s text COLLATE pg_catalog."default",
    state_l text COLLATE pg_catalog."default",
    revenue text COLLATE pg_catalog."default",
    yrs_on_list double precision,
    industry text COLLATE pg_catalog."default",
    metro text COLLATE pg_catalog."default",
    "Sector" text COLLATE pg_catalog."default"
);

CREATE TABLE IF NOT EXISTS public."Sector"
(
    sector_id text COLLATE pg_catalog."default",
    "Sector" text COLLATE pg_catalog."default"
);

ALTER TABLE IF EXISTS public."BLS"
    ADD CONSTRAINT "fky_Sector" FOREIGN KEY ("Sector")
    REFERENCES public."Sector" ("Sector") MATCH SIMPLE
    ON UPDATE NO ACTION
    ON DELETE NO ACTION
    NOT VALID;


ALTER TABLE IF EXISTS public."Companies"
    ADD CONSTRAINT fky_sector_company FOREIGN KEY ("Sector")
    REFERENCES public."Sector" ("Sector") MATCH SIMPLE
    ON UPDATE NO ACTION
    ON DELETE NO ACTION
    NOT VALID;

END;